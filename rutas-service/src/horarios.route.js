...
router.get("/horarios/buscar", async(req,res)=>{
    try{
        ...
        const resultado = await pool.query( `SELECT  id, ruta_id, parada_id, hora_paso FROM horarios WHERE ruta_id=$1 AND parada_id=$2 ORDER BY hora_paso `, [ ruta, parada ] );
        res.json(resultado.rows);
    }catch(error){
        console.error(error);
        res.status(500).json({
